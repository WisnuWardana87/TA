                ...
                <div class="col-12">
                    <div class="card-header">
                        <div class="card-tools">
                            <a><button type="button" class="btn btn-block btn-success tombolTambahData" data-toggle="modal" data-target="#dataFakultas"><i class="fas fa-file-export"></i> Eksport Data</button></a>
                        </div>
                    </div>
                    <div class="card">
                        <!-- /.card-header -->
                                    ...
                                    foreach ($fakultas as $val) { ?>
                                        <tr>
                                            ...
                                            <td class="text-center"><a>
                                                    <div class="btn btn-sm btn-primary tampilFakultasEdit" data-toggle="modal" data-target="#dataFakultas" data-id="<?= $val['id_fakultas']; ?>"><i class="fas fa-fw fa-edit"></i></div>
                                                </a></td>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="dataFakultasLabel">Tambah Data</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <form action="<?= base_url('MasterData/save_register_fakultas'); ?>" method="post">
...
<script>
    $('.tombolTambahData').on('click', function() {
        $('#dataFakultasLabel').html('Tambah Data');
        $('.modal-footer button[type=submit]').html('Tambah Data');


    })
    $('.tampilFakultasEdit').on('click', function() {
        $('#dataFakultasLabel').html('Edit Data');
        $('.modal-footer button[type=submit]').html('Edit Data');

        const id_fakultas = $(this).data('id_fakultas');
        $.ajax({
